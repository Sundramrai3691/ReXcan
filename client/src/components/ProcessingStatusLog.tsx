...
const ProcessingStatusLog = ({
  pythonJobId,
  onComplete,
  showHeader = true,
  compact = false,
}: ProcessingStatusLogProps) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if (isNotFound) {
            ...
            setJobNotFound(true);
            setError(
              "Job not found. The processing job may have expired or been cleared.",
            );
  ...
  const calculateProgress = (
    ...
    const ocrLog = logEntries.find((log) =>
      log.message.includes("OCR completed"),
    );
    const heuristicsLog = logEntries.find((log) =>
      log.message.includes("Heuristic extraction completed"),
    );
    const llmLog = logEntries.find((log) =>
      log.message.includes("LLM extraction completed"),
    );
  ...
  return (
    <div
      className={`bg-white rounded-lg ${compact ? "p-4" : "p-6"} border border-gray-200 shadow-sm`}
    >
            ...
            <div className="grid grid-cols-3 gap-3 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
              {metrics.ocrTime && (
                <div>
                  ...
                  <div className="text-sm font-medium text-gray-900">
                    {formatDuration(metrics.ocrTime)}
                  </div>
                </div>
              )}
              {metrics.heuristicsTime && (
                <div>
                  ...
                  <div className="text-sm font-medium text-gray-900">
                    {formatDuration(metrics.heuristicsTime)}
                  </div>
                </div>
              )}
              {metrics.llmTime && (
                <div>
                  ...
                  <div className="text-sm font-medium text-gray-900">
                    {formatDuration(metrics.llmTime)}
                  </div>
                </div>
              )}
              {metrics.totalTime && (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-yellow-700 mt-1">
                {error ||
                  "The processing job may have expired or been cleared from the system."}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Logs Container */}
      <div
        className={`${compact ? "max-h-64" : "max-h-96"} overflow-y-auto space-y-0.5 bg-gray-50 rounded border border-gray-200 p-3`}
      >
            ...
            <div className="space-y-0.5">
              {logs.map((log, index) => (
                ...
                >
                  ...
                  <span className="w-4 flex-shrink-0 text-sm flex items-center justify-center mt-0.5">
                    {getLogIcon(log.level)}
                  </span>
                  <span className="flex-1 break-words text-sm leading-relaxed text-gray-700">
                    {log.message}
                  </span>
                </div>
              ))}
