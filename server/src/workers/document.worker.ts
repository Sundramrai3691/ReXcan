...
// Lazy initialization pattern for worker
let documentWorkerInstance: Worker<DocumentJobData> | null = null;

export const getDocumentWorker = (): Worker<DocumentJobData> => {
  if (!documentWorkerInstance) {
    documentWorkerInstance = new Worker<DocumentJobData>(
    ...
    documentWorkerInstance.on('failed', (job, err) => {
      logger.error(`Job ${job?.id} failed:`, err);
    });

    documentWorkerInstance.on('error', (error) => {
...
// Create worker through lazy initialization
export const documentWorker = new Proxy(
  {} as Worker<DocumentJobData>,
  {
    get: (_, prop) => {
      return (getDocumentWorker() as any)[prop];
    },
  }
);
...
process.on('SIGTERM', async () => {
  ...
  const worker = documentWorkerInstance;
  if (worker) {
    await worker.close();
  }
  process.exit(0);
